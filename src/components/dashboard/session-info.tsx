...
import { CalendarDays, Clock, Fingerprint, Globe, Monitor, Shield } from 'lucide-react';
...
export default function SessionInfo() {
  ...
  const infoItems = [
    ...
    },
    {
      ...
      value: data.fingerprintHash ? `${data.fingerprintHash.slice(0, 16)}...` : 'Not captured',
      badge: data.fingerprintHash ? 'Bound' : 'Unbound',
      badgeVariant: data.fingerprintHash ? ('default' as const) : ('secondary' as const),
    },
    {
      icon: CalendarDays,
      label: 'Created At',
    ...
    },
    {
      icon: Clock,
      label: 'Expires At',
      value: data.expiresAt ? new Date(data.expiresAt).toLocaleString() : 'Unknown',
    },
  ];
